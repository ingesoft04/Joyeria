...
export class Hand3D{
  ...
  buildHand(){
    ...
    this.mesh(new THREE.CapsuleGeometry(.45,.9,10,20),0,-1.45,0,[1,1,.72]);
  ...
  setJewelry({item,thread,look,weave,symbol,gender}){
    ...
    this.jewelry.clear();const ring=item==='Anillo',major=ring?.21:.52,tube=ring?.045:(weave==='double'?.09:.065),threadMaterial=new THREE.MeshPhysicalMaterial({color:thread,roughness:.68,clearcoat:.2});
    const cord=new THREE.Mesh(new THREE.TorusGeometry(major,tube,12,64),threadMaterial);cord.rotation.x=ring?Math.PI/2:0;cord.position.set(ring?.22:0,ring?.92:-1.02,ring?0:.02);this.jewelry.add(cord);
    const gold=new THREE.MeshPhysicalMaterial({color:0xd6a33b,metalness:.82,roughness:.18,clearcoat:1});const count=ring?8:13;
    for(let i=0;i<count;i++){const a=i/count*Math.PI*2,r=major,diamond=look==='diamond'||look==='mixed'&&i%2;const bead=new THREE.Mesh(diamond?new THREE.OctahedronGeometry(ring?.065:.095,1):new THREE.SphereGeometry(ring?.065:.095,16,12),gold);if(ring){bead.position.set(.22+Math.cos(a)*r,.92,Math.sin(a)*r);bead.rotation.x=Math.PI/2}else bead.position.set(Math.cos(a)*r,-1.02+Math.sin(a)*r,.05);this.jewelry.add(bead)}
    if(symbol){const charm=new THREE.Mesh(new THREE.SphereGeometry(ring?.1:.16,18,14),gold);charm.position.set(ring?.22:0,ring?.55:-1.68,.08);this.jewelry.add(charm)}
  }
